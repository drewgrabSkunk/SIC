...
tech.transform.short2 <- function(URL, ticker) {
  output.file <-
    paste(
  ...
  tech$MA.cross.3[2:36] <- NA
  for ( i in 23:length(tech$Close)) {
    tech$MA.cross.3[i] <- tech$MA.cross.up[i] + tech$MA.cross.up[i-1] + tech$MA.cross.up[i-2]}
  
  tech$MA.cross.down.3[1] <- NA
  tech$MA.cross.down.3[2:40] <- NA
  for ( i in 23:length(tech$Close)) {
    tech$MA.cross.down.3[i] <- tech$MA.cross.down[i] + tech$MA.cross.down[i-1] + tech$MA.cross.down[i-2]}
  ...
  tech$buy.three.cross[2:37] <- NA
  for (i in 38:length(tech$Close)) {
  ...
  #Buy Triple Cross down experiment 3#
  tech$buy.three.cross.exp[1] <- NA
  tech$buy.three.cross.exp[2:40] <- NA
  for (i in 41:length(tech$Close)) {
    if((tech$bb.kc[i] > 1) & (tech$CCI.cross.3[i] > 1) & (tech$DI.cross.3[i] > 1) & (tech$MA.cross.down.3[i] > 1)) {
      tech$buy.three.cross.exp[i] <- 1}
    else { tech$buy.three.cross.exp[i] <- 0}
  }
  
  #Triple Cross Short#
  tech$short.three.cross[1] <- NA
  tech$short.three.cross[2:40] <- NA
  for (i in 41:length(tech$Close)) {
    if((tech$bb.kc[i] == 1) & (tech$CCI.cross.neg.3[i] == 1) & (tech$DI.cross.down.3[i] == 1) & (tech$MA.cross.down.3[i] == 1)) {
      tech$short.three.cross[i] <- 1}
    else { tech$short.three.cross[i] <- 0}
  }
  
  #Buy Roll DI Cross 3 (DI Roll = 6 Days)#
  tech$buy.roll.DI[1] <- NA
  tech$buy.roll.DI[2:42] <- NA
  for (i in 43:length(tech$Close)) {
  ...
  #Buy Roll DI Cross 3 (DI Roll = 6 Days) down experiment#
  tech$buy.roll.DI.exp[1] <- NA
  tech$buy.roll.DI.exp[2:40] <- NA
  for (i in 41:length(tech$Close)) {
    if((tech$bb.kc[i] > 0) & (tech$CCI.cross.3[i] > 0) & (tech$MA.cross.down.3[i] > 0) & (tech$DI.cross.3[i] > 0)
       & (tech$DI.cross.3[i-1] > 0) & (tech$DI.cross.3[i-2] > 0) & (tech$DI.cross.3[i-3] > 0)
       & (tech$DI.cross.3[i-4] > 0) & (tech$DI.cross.3[i-5] > 0)) {
      tech$buy.roll.DI.exp[i] <- 1}
    else { tech$buy.roll.DI.exp[i] <- 0}
  }
  
  #Buy MA Above#
  tech$buy.MA.above[1] <- NA
  tech$buy.MA.above[2:37] <- NA
  for (i in 38:length(tech$Close)) {
  ...
  #Short signal#
  tech$short[1:length(tech$close)] <- 1
  #tech$short[2:45] <- NA
  #for (i in 46:length(tech$close)) {
    #if((tech$short.ma.inverse[i] == 1)) #|| (tech$short.three.cross[i] == 1))#
      # {tech$short[i] <- 1}
    #else {tech$short[i] <- 0}
  #}
  ...
  #Holding Gain on short and next cover#
